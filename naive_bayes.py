...
class Classifier:
    def __init__(self, bucketPrefix, testBucketNumber=None, dataFormat=None):
        if not testBucketNumber:
            self.load_all(bucketPrefix)
        else:
            self.load_buckets(bucketPrefix, testBucketNumber, dataFormat)
    ...
    def load_all(self, filename):
        ...
        self.counts = counts
#        self.visualize_counts(counts)
        # TODO TODO
        # VISUALIZE COUNTS

        self.compute(classes, counts, total)


    def load_buckets(self, bucketPrefix, testBucketNumber, dataFormat):
...
#        self.visualize_counts(counts)
    ...
    def visualize_counts(self, counts):
        print counts

    def compute(self, classes, counts, total):
        #
              ...
              for (col, valueCounts) in columns.items():
                  ...
                  for (attrValue, count) in valueCounts.items():
                      self.conditional[category][col][attrValue] = (
                          float(count) / classes[category])
        self.tmp =  counts
      #  print self.conditional

    def testFromFile(self, filename):
        return self.testBucket(0, 0, filename)

    def testBucket(self, bucketPrefix, bucketNumber, filename=None):
...
def tenfold(bucketPrefix, dataFormat="class\tattr\tattr\tattr\tattr\tattr\tattr\tattr\tattr\tattr\tattr\tattr\tattr\tattr\tattr\tattr\tattr"):
    results = {}
    for i in range(1, 11):
